...
const{PROVIDERS,generateComment}=require('./llm');
initLogger();
...
function advanceNarr(r){const oldNarrId=r.turnOrder[r.narratorIndex];r.turnOrder=r.turnOrder.filter(pid=>r.players.has(pid));if(r.turnOrder.length===0)return null;
  // Re-anchor narratorIndex to the current narrator after filter
  const newIdx=r.turnOrder.indexOf(oldNarrId);r.narratorIndex=newIdx>=0?(newIdx+1)%r.turnOrder.length:r.narratorIndex%r.turnOrder.length;
  let t=0;while(!r.players.get(r.turnOrder[r.narratorIndex])?.connected&&t<r.turnOrder.length){r.narratorIndex=(r.narratorIndex+1)%r.turnOrder.length;t++;}clearInact(r);r.vetoVotes=new Map();return getNarr(r);}
...
function applyResult(r,io,rc,approved){
  ...
  if(v.type==='integrate'){
    ...
    if(approved){if(v.provisionalIg)v.provisionalIg._provisional=false;init.hand=init.hand.filter(c=>c.id!==v.concept.id);init.integrated.push(v.concept);if(v.prevNarrIdx!=null){const oldN=r.players.get(r.turnOrder[v.prevNarrIdx]);if(oldN&&r.deck.length>0)oldN.hand.push(r.deck.shift());}addLog(r,'interrupt',init.id,`✅ Interrupción confirmada — ${cardLabel(v.concept)}`);io.to(rc).emit('interrupt-alert',{playerName:init.name,conceptName:v.concept.name,conceptType:v.concept.type,isInterruption:v.concept.isInterruption});const prevN=v.prevNarrIdx!=null?r.players.get(r.turnOrder[v.prevNarrIdx])?.name:'?';triggerComment(r,io,rc,{type:'interrupt',playerName:init.name,prevNarrator:prevN,cardName:v.concept.name,cardType:TL[v.concept.type]||v.concept.type,fragment:v.fragment?.text||''});}
    else{r.narratorIndex=v.prevNarrIdx!=null?v.prevNarrIdx:r.narratorIndex;r.integrations=r.integrations.filter(ig=>ig!==v.provisionalIg);r.vetoVotes=new Map();init.hand=init.hand.filter(c=>c.id!==v.concept.id);r.discardPile.push(v.concept);if(r.deck.length>0)init.hand.push(r.deck.shift());const rewindTo=r.storyBeforeVote||r.story;r.story=rewindTo;io.to(rc).emit('story-rewind',{text:rewindTo});const nn=getNarr(r);addLog(r,'interrupt',init.id,`❌ ${init.name}: interrupción rechazada — pierde ${cardLabel(v.concept)} +1 carta`);io.to(rc).emit('narrator-changed',{narratorId:nn.id,narratorName:nn.name});triggerComment(r,io,rc,{type:'interrupt-rejected',playerName:init.name,cardName:v.concept.name,cardType:TL[v.concept.type]||v.concept.type});}
    resetInact(r,io,rc);resetCardPlay(r,io,rc);
  }else if(v.type==='ending'){
    if(approved){r.phase='finished';r.winnerId=v.initiatorId;addLog(r,'ending',init.id,`🏆 ¡${init.name} gana!`);clearInact(r);clearCardPlay(r);triggerComment(r,io,rc,{type:'victory',playerName:init.name,endingText:v.concept?.text||v.concept?.name||''});}
    else{
...
// ═══ COMMENTARY — AI commentator triggers ═══
function getRecentStory(r,maxLen=500){const s=r.story||'';return s.length>maxLen?'...'+s.slice(-maxLen):s;}
function triggerComment(r,io,rc,event){
  if(!r._aiConfig||!r._aiConfig.enabled)return;
  const now=Date.now();if(r._lastComment&&now-r._lastComment<12000)return; // rate limit 12s
  r._lastComment=now;
  const context={recentStory:getRecentStory(r),fullStory:r.story||'',playerCount:r.players.size,totalCards:r.integrations.length};
  let full='';
  io.to(rc).emit('commentator-start',{eventType:event.type});
  generateComment(r._aiConfig.provider,r._aiConfig.apiKey,r._aiConfig.model,event,context,
    (chunk,accumulated)=>{full=accumulated;io.to(rc).emit('commentator-chunk',{text:chunk,full:accumulated});},
    (finalText)=>{io.to(rc).emit('commentator-done',{text:finalText});},
    (err)=>{io.to(rc).emit('commentator-error',{error:err});sessionLog(rc,'warn','commentator-error',{error:err});}
  );
}
...
app.get('/api/providers',(_,res)=>{const out={};for(const[k,v]of Object.entries(PROVIDERS))out[k]={name:v.name,models:v.models,keyUrl:v.keyUrl};res.json(out);});
...
io.on('connection',(socket)=>{
  ...
  socket.on('set-ai-config',({provider,apiKey,model,enabled},cb)=>{const i=stp.get(socket.id);if(!i)return cb?.({error:'?'});const r=rooms.get(i.roomCode);if(!r)return cb?.({error:'?'});const isPlayerHost=r.players.get(i.playerId)?.isHost;const isSpecHost=isHostSpectator(r,i.playerId);if(!isPlayerHost&&!isSpecHost)return cb?.({error:'Solo host'});if(enabled&&provider&&apiKey&&model){r._aiConfig={provider,apiKey,model,enabled:true};cb?.({success:true});}else{r._aiConfig=null;cb?.({success:true});}});
  ...
  socket.on('story-update',({text})=>{const i=stp.get(socket.id);if(!i)return;const r=rooms.get(i.roomCode);if(!r||r.phase!=='playing'||getNarrId(r)!==i.playerId)return;if(typeof text!=='string'||text.length>50000)return;const frozen=effectiveFrozen(r);const prefix=r.story.substring(0,frozen);if(text.length<frozen||text.substring(0,frozen)!==prefix)return;r.story=text;r.inactSec=0;sessionLog(i.roomCode,'debug','story-update',{len:text.length},socket.id);socket.to(i.roomCode).emit('story-updated',{text});
    // Commentary on writing — every ~150 new characters
    if(r._aiConfig?.enabled){if(!r._commentStoryLen)r._commentStoryLen=0;const newChars=text.length-r._commentStoryLen;if(newChars>=150){r._commentStoryLen=text.length;const narr=getNarr(r);triggerComment(r,io,i.roomCode,{type:'narrating',description:`${narr?.name||'El narrador'} está escribiendo la historia.`,playerName:narr?.name||'?'});}}});
  socket.on('integrate-concept',({conceptId,fragment,justification},cb)=>{const i=stp.get(socket.id);if(!i)return cb?.({error:'?'});const r=rooms.get(i.roomCode);if(!r||r.phase!=='playing'||getNarrId(r)!==i.playerId)return cb?.({error:'No'});if(r.currentVote)return cb?.({error:'Voto en curso'});if(r.interruptWindow)return cb?.({error:'Ventana de interrupción activa'});if(!fragment||typeof fragment.text!=='string'||typeof fragment.start!=='number'||typeof fragment.end!=='number')return cb?.({error:'Fragmento inválido'});const p=r.players.get(i.playerId);if(!p)return cb?.({error:'?'});const c=p.hand.find(x=>x.id===conceptId);if(!c)return cb?.({error:'Carta no encontrada'});sessionLog(i.roomCode,'info','integrate-concept',{player:p.name,card:c.name,type:c.type,isEnding:!!c.isEnding,fragment:fragment?.text?.substring(0,60)},socket.id);clearCardPlay(r);startVote(r,io,i.roomCode,c.isEnding?'ending':'integrate',i.playerId,c,fragment,justification);cb?.({success:true});});
  socket.on('interrupt',({conceptId,fragment,justification},cb)=>{const i=stp.get(socket.id);if(!i)return cb?.({error:'?'});const r=rooms.get(i.roomCode);if(!r||r.phase!=='playing'||getNarrId(r)===i.playerId)return cb?.({error:'No'});if(r.currentVote)return cb?.({error:'Voto en curso'});if(r.interruptWindow)return cb?.({error:'Ventana de interrupción activa'});if(!fragment||typeof fragment.text!=='string'||typeof fragment.start!=='number'||typeof fragment.end!=='number')return cb?.({error:'Fragmento inválido'});const p=r.players.get(i.playerId);if(!p)return cb?.({error:'No eres jugador'});const c=p.hand.find(x=>x.id===conceptId);if(!c)return cb?.({error:'Carta no encontrada'});if(c.isEnding)return cb?.({error:'Final no interrumpe'});sessionLog(i.roomCode,'info','interrupt',{player:p.name,card:c.name,type:c.type,fragment:fragment?.text?.substring(0,60)},socket.id);clearCardPlay(r);startVote(r,io,i.roomCode,'interrupt',i.playerId,c,fragment,justification);cb?.({success:true});});
  ...
  socket.on('pass-turn',({discardConceptId},cb)=>{const i=stp.get(socket.id);if(!i)return cb?.({error:'?'});const r=rooms.get(i.roomCode);if(!r||r.phase!=='playing'||getNarrId(r)!==i.playerId)return cb?.({error:'No'});if(r.currentVote)return cb?.({error:'Voto en curso'});if(r.interruptWindow)closeInterruptWindow(r,io,i.roomCode);const p=r.players.get(i.playerId);sessionLog(i.roomCode,'info','pass-turn',{player:p.name,discardConceptId:discardConceptId||null},socket.id);if(discardConceptId){const idx=p.hand.findIndex(c=>c.id===discardConceptId&&!c.isEnding);if(idx>=0){const d=p.hand.splice(idx,1)[0];r.discardPile.push(d);if(r.deck.length>0)p.hand.push(r.deck.shift());addLog(r,'pass',p.id,`🔄 ${p.name} descarta ${cardLabel(d)}, roba 1`);}}else{if(r.deck.length>0){p.hand.push(r.deck.shift());addLog(r,'pass',p.id,`🔄 ${p.name} no tiene cartas narrativas — roba 1`);}else addLog(r,'pass',p.id,`${p.name} pasa`);}const n=advanceNarr(r);if(n){addLog(r,'narrator',n.id,`✍️ ${n.name} narra`);io.to(i.roomCode).emit('narrator-changed',{narratorId:n.id,narratorName:n.name});}triggerComment(r,io,i.roomCode,{type:'pass',playerName:p.name});cb?.({success:true});broadcastGS(io,i.roomCode);resetInact(r,io,i.roomCode);resetCardPlay(r,io,i.roomCode);});
  socket.on('veto-narrator',({reason},cb)=>{const i=stp.get(socket.id);if(!i)return cb?.({error:'?'});const r=rooms.get(i.roomCode);if(!r||r.phase!=='playing'||getNarrId(r)===i.playerId||r.currentVote)return cb?.({error:'No'});const p=r.players.get(i.playerId);if(!p)return cb?.({error:'No eres jugador'});r.vetoVotes.set(i.playerId,true);sessionLog(i.roomCode,'info','veto-narrator',{player:p.name,reason:reason?.trim()||'',count:r.vetoVotes.size,threshold:Math.ceil((r.players.size-1)/2)},socket.id);const reasonStr=reason?.trim()?` — «${reason.trim()}»`:'';addLog(r,'system',i.playerId,`🚫 ${p.name} veta${reasonStr} (${r.vetoVotes.size}/${Math.ceil((r.players.size-1)/2)})`);if(checkVeto(r)){if(r.interruptWindow)closeInterruptWindow(r,io,i.roomCode);const narr=getNarr(r);const narrName=narr?.name||'?';if(narr){if(r.deck.length>0)narr.hand.push(r.deck.shift());addLog(r,'system',null,`🚫 Veto! ${narr.name} pierde turno +1 carta`);}const n=advanceNarr(r);if(n){addLog(r,'narrator',n.id,`✍️ ${n.name} narra`);io.to(i.roomCode).emit('narrator-changed',{narratorId:n.id,narratorName:n.name});}triggerComment(r,io,i.roomCode,{type:'veto',narratorName:narrName,reason:reason?.trim()||''});resetInact(r,io,i.roomCode);resetCardPlay(r,io,i.roomCode);}cb?.({success:true});broadcastGS(io,i.roomCode);});
  socket.on('restart-game',(_,cb)=>{const i=stp.get(socket.id);if(!i)return cb?.({error:'?'});const r=rooms.get(i.roomCode);if(!r)return cb?.({error:'?'});const isPlayerHost=r.players.get(i.playerId)?.isHost;const isSpecHost=isHostSpectator(r,i.playerId);if(!isPlayerHost&&!isSpecHost)return cb?.({error:'No'});r.phase='lobby';clearInact(r);clearCardPlay(r);if(r.currentVote?.timer)clearInterval(r.currentVote.timer);r.currentVote=null;if(r.interruptWindow?.timer)clearInterval(r.interruptWindow.timer);r.interruptWindow=null;r.story='';r.integrations=[];r.actionLog=[];r.winnerId=null;r.vetoVotes=new Map();r.turnOrder=[];r.narratorIndex=0;r.deck=[];r.endingsDeck=[];r.discardPile=[];r.storyBeforeVote=undefined;r._lastComment=0;const hostSpecs=[...r.spectators.entries()].filter(([,s])=>s.isHost);r.spectators=new Map(hostSpecs);for(const p of r.players.values()){p.hand=[];p.integrated=[];}sessionLog(i.roomCode,'info','game-restarted',{});cb?.({success:true});broadcastLobby(io,i.roomCode);});
