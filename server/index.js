...
function triggerComment(r,io,rc,event){
  ...
  const now=Date.now();if(r._lastComment&&now-r._lastComment<25000)return; // rate limit 25s
  r._lastComment=now;
...
io.on('connection',(socket)=>{
  ...
  socket.on('story-update',({text})=>{const i=stp.get(socket.id);if(!i)return;const r=rooms.get(i.roomCode);if(!r||r.phase!=='playing'||getNarrId(r)!==i.playerId)return;if(typeof text!=='string'||text.length>50000)return;const frozen=effectiveFrozen(r);const prefix=r.story.substring(0,frozen);if(text.length<frozen||text.substring(0,frozen)!==prefix)return;r.story=text;r.inactSec=0;sessionLog(i.roomCode,'debug','story-update',{len:text.length},socket.id);socket.to(i.roomCode).emit('story-updated',{text});
    // Commentary on writing — every ~300 new characters
    if(r._aiConfig?.enabled){if(!r._commentStoryLen)r._commentStoryLen=0;const newChars=text.length-r._commentStoryLen;if(newChars>=300){r._commentStoryLen=text.length;const narr=getNarr(r);triggerComment(r,io,i.roomCode,{type:'narrating',description:`${narr?.name||'El narrador'} está escribiendo la historia.`,playerName:narr?.name||'?'});}}});
